 ...
 *
 * Features:
 * - Enhanced UI with modern styling and dark mode toggle
...
function get_redis_connection() {
    static $redis = null;
    if ($redis === null && defined('WP_REDIS_HOST') && class_exists('Redis')) {
        try {
            $redis = new Redis();
...
function get_page_speed_score() {
    $api_key = 'YOUR_GOOGLE_PAGESPEED_API_KEY'; // Thay bằng API key của bạn
    if (empty($api_key) || $api_key === 'YOUR_GOOGLE_PAGESPEED_API_KEY') {
        return ['score' => 'N/A', 'load_time' => 'N/A', 'status' => 'error', 'message' => 'Google PageSpeed API key not configured'];
    }
    ...
    try {
        ...
        if (is_wp_error($response)) {
            return ['score' => 'N/A', 'load_time' => 'N/A', 'status' => 'error', 'message' => $response->get_error_message()];
        }
        ...
        return ['score' => 'N/A', 'load_time' => 'N/A', 'status' => 'error', 'message' => $e->getMessage()];
    }
}
...
function get_server_resources() {
    $resources = [
        'cpu_usage' => 'N/A (Function not available on this server)',
        'ram_usage' => 'N/A (Function not available)',
        'disk_space' => 'N/A (Function not available)'
    ];
...
function check_cron_status() {
    ...
    return [
        ...
        'next_scheduled' => $next_scheduled ? date('Y-m-d H:i:s', $next_scheduled) : 'N/A',
        'disabled' => defined('DISABLE_WP_CRON') && DISABLE_WP_CRON ? 'Yes' : 'No'
    ];
}

function generate_health_recommendations($page_speed, $cache_status, $server_resources, $wp_config, $redis_info, $cron_status) {
    $recommendations = [];
    
    // Page Speed Recommendations
    if ($page_speed['status'] === 'error') {
        $recommendations[] = 'Cấu hình Google PageSpeed API key để kiểm tra tốc độ tải trang.';
    } elseif ($page_speed['status'] === 'critical' || $page_speed['status'] === 'warning') {
    ...
    if ($redis_info['status'] !== 'Connected') {
        $recommendations[] = 'Kiểm tra cấu hình Redis (host: ' . (defined('WP_REDIS_HOST') ? WP_REDIS_HOST : 'N/A') . ', port: ' . (defined('WP_REDIS_PORT') ? WP_REDIS_PORT : 'N/A') . ').';
    ...
    if ($cron_status['disabled'] === 'Yes') {
        $recommendations[] = 'WP Cron bị vô hiệu hóa. Xem xét sử dụng cron hệ thống để cải thiện hiệu suất.';
    } elseif ($cron_status['total_jobs'] > 100) {
        $recommendations[] = 'Số lượng cron jobs lớn (' . $cron_status['total_jobs'] . '). Xem xét tối ưu hóa hoặc xóa các job không cần thiết.';
    }
...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] === 'health_check') {
        ...
        $health_recommendations = generate_health_recommendations($page_speed, $cache_status, $server_resources, $wp_config, $redis_info, $cron_status);
    ...
    if ($_GET['ajax'] === 'load_history') {
        $log_file = WP_CONTENT_DIR . '/debug-memory.log';
        $logs = [];
        if (file_exists($log_file)) {
            $lines = file($log_file, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
            $logs = array_map('json_decode', array_slice($lines, -50)); // Giới hạn 50 dòng cuối
        }
...
<body>
        ...
        <div class="content">
            ...
            <div id="health" class="tab-content">
                ...
                <div class="stats-grid">
                    ...
                    <div class="stat-card">
                        <h3>⏰ Cron Jobs</h3>
                        <div class="stat-value" id="cron-status">Đang kiểm tra...</div>
                        <small>Thông tin về WordPress Cron</small>
                    </div>
                </div>
    ...
    <script>
        ...
        function cleanTransients() {
            if (confirm('Bạn có chắc muốn xóa các transient hết hạn?')) {
                makeAjaxRequest('clean_transients');
            }
        }
        
        function cleanCartFragments() {
            if (confirm('Bạn có chắc muốn xóa các cart fragments?')) {
                makeAjaxRequest('clean_cart_fragments');
            }
        }
        
        function optimizeDatabase() {
            if (confirm('Bạn có chắc muốn tối ưu hóa tất cả các bảng cơ sở dữ liệu? Việc này có thể mất vài phút.')) {
            ...
            fetch(window.location.href + '?ajax=health_check')
                ...
                .then(data => {
                    ...
                    pageSpeedEl.innerHTML = data.page_speed.score !== 'N/A' 
                        ...
                        : `Không thể kiểm tra: ${data.page_speed.message || 'Error'}`;
                    ...
                    cronEl.innerHTML = Object.entries(data.cron_status)
                        .map(([key, value]) => `${key.replace('_', ' ').toUpperCase()}: ${value}`)
                        .join('<br>');
        ...
        function loadHistory() {
            ...
            fetch(window.location.href + '?ajax=load_history')
                .then(response => response.json())
                .then(data => {
                    hideLoading();
                    ...
                    if (data.success && data.logs.length) {
                        historyEl.innerHTML = '<ul>' + 
                            data.logs.map(log => 
                                `<li>${log.timestamp}: ${format_memory(log.memory_used)} used, ${format_memory(log.memory_peak)} peak</li>`
                            ).join('') +
                            '</ul>';
                    } else {
                        historyEl.innerHTML = '<p>Không có dữ liệu lịch sử.</p>';
                    }
                })
                .catch(error => {
                    hideLoading();
                    console.error('History load error:', error);
                    document.getElementById('history-content').innerHTML = '<p>Lỗi khi tải lịch sử.</p>';
                });
        }
        ...
        function exportPDF() {
            ...
            const doc = new jsPDF();
            doc.setFontSize(16);
            doc.text('WordPress Debug Report', 10, 10);
            doc.setFontSize(12);
            doc.text(`Timestamp: ${new Date().toISOString()}`, 10, 20);
            doc.text(`Memory Used: <?php echo format_memory($memory_used); ?>`, 10, 30);
            doc.text(`Peak Memory: <?php echo format_memory($memory_peak); ?>`, 10, 40);
            doc.text(`Memory Limit: <?php echo esc_js($memory_limit); ?>`, 10, 50);
            doc.text(`WordPress Version: <?php echo esc_js(get_bloginfo('version')); ?>`, 10, 60);
            doc.text(`PHP Version: <?php echo esc_js(phpversion()); ?>`, 10, 70);
            doc.text(`Active Plugins: <?php echo count(get_option('active_plugins', [])); ?>`, 10, 80);
            doc.text(`Database Size: <?php echo isset($total_size) ? round($total_size, 2) : 0; ?> MB`, 10, 90);
            doc.save(`wordpress-debug-report-${new Date().toISOString().split('T')[0]}.pdf`);
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            exportJsonBtn.style.marginTop = '15px';
            exportJsonBtn.style.marginLeft = '10px';
            exportJsonBtn.onclick = exportReport;
            header.appendChild(exportJsonBtn);
            
            const exportPdfBtn = document.createElement('button');
            exportPdfBtn.innerHTML = '📄 Export PDF';
            exportPdfBtn.className = 'btn btn-success';
            exportPdfBtn.style.marginTop = '15px';
            exportPdfBtn.style.marginLeft = '10px';
        ...
        function switchTabByIndex(index) {
            const tabs = ['overview', 'database', 'plugins', 'optimization', 'security', 'health', 'history'];
            if (tabs[index]) {
...
?>
<!--
WordPress Memory Debug Tool v2.3
Features implemented:
✅ Modern responsive UI with dark mode toggle
✅ Tabbed interface with Health Check and History tabs
✅ Real-time memory monitoring
✅ Database optimization tools
✅ Plugin analysis and management
✅ Security checks and recommendations
✅ Export functionality for JSON and PDF reports
✅ AJAX operations for optimization tasks
✅ Keyboard shortcuts (Ctrl+1-7 for tabs, Ctrl+E for JSON, Ctrl+P for PDF)
✅ Performance recommendations
✅ Mobile-friendly responsive design
✅ Print-friendly styling
✅ Memory usage logging with size limit
...
6. Replace YOUR_GOOGLE_PAGESPEED_API_KEY with your API key for PageSpeed Insights
7. Ensure Redis is properly configured and the Redis PHP extension is installed
