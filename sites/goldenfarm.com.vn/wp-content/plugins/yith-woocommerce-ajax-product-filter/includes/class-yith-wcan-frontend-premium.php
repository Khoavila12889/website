...
if ( ! class_exists( 'YITH_WCAN_Frontend_Premium' ) ) {
	...
	class YITH_WCAN_Frontend_Premium extends YITH_WCAN_Frontend {
		...
		public function filters_title( $preset = false ) {
			...
			if ( empty( $title ) ) {
				$title                      = apply_filters( 'yith_wcan_default_modal_title', _x( 'Lọc sản phẩm', '[FRONTEND] Default modal title - mobile only', 'yith-woocommerce-ajax-navigation' ) );
		...
		public function mobile_modal_opener( $preset = false ) {
			...
			$label  = apply_filters( 'yith_wcan_mobile_modal_opener_label', _x( 'Bộ lọc', '[FRONTEND] Label for the Filters button on mobile', 'yith-woocommerce-ajax-navigation' ) );
