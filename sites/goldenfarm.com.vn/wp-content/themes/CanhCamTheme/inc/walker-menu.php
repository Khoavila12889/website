...
class CleanMenuWalker extends Walker_Nav_Menu
{
	...
	function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
	{
		...
		$appearance_menu_image = get_field("appearance_menu_image", $item->ID);
		$appearance_menu_color = get_field("appearance_menu_color", $item->ID);
		...
		if($appearance_menu_color)
			$output .= '<li' . $class_names . ' style="color:' . $appearance_menu_color . '"' . '>';
		else
		...
		if ($appearance_menu_image) {
			$item_output .= '<a class="notranslate"' . $attributes . '>';
			$item_output .= '<img src="' . $appearance_menu_image['url'] . '" alt="' . $appearance_menu_image['alt'] . '">';
